...
All architectural and design decisions in this project — the data model, the Google-only auth approach, the access-control model, the sharing schema, the materialized-path/denormalized-aggregate strategy — were made by the repository's author. Claude (Anthropic) was used as a supporting tool throughout implementation: validating design hypotheses before committing to them, analyzing deployment and runtime error logs (e.g. the Alpine/OpenSSL-musl incompatibility with Prisma's engine, TypeScript build configuration issues), refactoring and cleaning up code, writing Bash setup scripts, and fixing syntax issues. Every generated change was reviewed and tested by the author before being pushed.
